...
object Embedding extends Serializable with LazyLogging {
    ...
    private def makeWord2VecModel(df: DataFrame,
                                  inputColName: String,
  ...
  private def generateSynonyms(df: DataFrame, matchesModel: Word2VecModel)(implicit sparkSession: SparkSession) = {
  ...
  private def generateWord2VecModel(df: DataFrame)(implicit sparkSession: SparkSession) = {
    ...
    val matchesPerPMID = mDF.groupBy($"pmid")
